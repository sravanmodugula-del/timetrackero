...
// Removed Redis imports - using optimized memory store
...
export function getSession() {
  ...
  // Create optimized memory store
  const memoryStore = new session.MemoryStore();
  
  // Add minimal debugging only in development
  if (!isProduction) {
    ...
    memoryStore.get = function(sessionId, callback) {
      originalGet(sessionId, (err, session) => {
        if (!session) {
          console.log(`🔍 Session ${sessionId} not found in store`);
        }
  ...
  console.log(`🔐 Using optimized memory session store (${isProduction ? 'production' : 'development'} mode)`);
  ...
  const sessionConfig = {
    ...
    rolling: true, // Enable rolling to extend session on activity
    cookie: {
      httpOnly: false, // Disable for IIS reverse proxy compatibility
      secure: false, // Disable for IIS reverse proxy compatibility - HTTPS termination at IIS
      maxAge: sessionTtl,
      sameSite: 'lax' as const, // Change from 'none' to 'lax' for same-site navigation
      ...
      try {
        ...
        req.session.regenerate((regenerateErr) => {
          ...
          req.session.save((saveErr) => {
            ...
            // Method 1: Direct cookie setting with corrected SameSite
            ...
            // Method 2: Set-Cookie header directly with corrected SameSite
            res.setHeader('Set-Cookie', [
              `${sessionCookieName}=${sessionCookieValue}; Path=/; HttpOnly=false; Secure=false; SameSite=Lax; Max-Age=${7 * 24 * 60 * 60}`
            ]);

...
export function getSession() {
  ...
  // For MS SQL Server on-premises deployment, use memory session store
  // MS SQL Server session stores require additional configuration that's not needed for this deployment

  // Default to memory session store (development or when PostgreSQL is not available)
  const sessionConfig = {
    ...
    cookie: {
      httpOnly: true, // Always true for security, even in development
      secure: true, // Always true since IIS provides HTTPS
      maxAge: sessionTtl,
      sameSite: 'lax', // Use 'lax' for better compatibility with IIS reverse proxy
      domain: undefined, // Remove domain restriction to fix session mismatch
      path: '/', // Explicitly set path
    },
    name: 'timetracker.sid',
    proxy: isProduction, // Trust proxy in production (IIS reverse proxy)
  };
